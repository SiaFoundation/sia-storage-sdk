...
class Sdk(_Sdk):
    ...
    async def upload_path(
        ...
        Prefer this to upload() for files on disk: the file is read on the
        SDK's runtime instead of crossing the language boundary once per chunk.
...
class PackedUpload(_PackedUpload):
    ...
    async def add_path(self, path: str) -> int:
        ...
        Prefer this to add() for files on disk: the file is read on the SDK's
        runtime instead of crossing the language boundary once per chunk.
